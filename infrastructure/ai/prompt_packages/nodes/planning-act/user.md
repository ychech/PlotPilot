...
请基于上面的压缩上下文，为这一幕规划 {chapter_count} 个章节。每章必须包含爽点标注和伏笔操作。

上下文使用规则：
- 只把 context 当作本幕规划材料，不要把其中的“变量说明”“字数说明”写进章节标题或大纲。
- 章节大纲必须使用人物 ID、地点 ID 时才引用；没有必要不要硬塞全部人物地点。
- 如果 context 中某项为空，以本幕信息和故事内核锁为准，不要另造题材或替换主角承诺。
